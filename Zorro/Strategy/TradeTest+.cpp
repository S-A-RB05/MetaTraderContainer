...
DLLFUNC int tradeAdapt(var Step)
{
...
void setLimit(var Factor)
{
	...
	if(OrderMode == 1) {
		OrderLimit = priceC(0) + Factor*Spread; 
		printf(" Limit %s",sftoa(OrderLimit,5));
	} else if(OrderMode == 2) {
		OrderLimit = priceC(0) + ifelse(Factor<0,-Spread,0.);
...
#ifdef PERCENTSTOP
	if(slider(2) > 0)
		Stop = Trail = roundto(0.01*priceC(0)*slider(2),PIP);
	else
		Stop = 0;
#else
...
DLLFUNC void click(int row,int col)
{
	...
	if(strstr(Text,"Auto Off")) {
		...
		printf("\n%s %s",Text,Asset);
		if(strstr(Text,"Buy Long"))
			call(1,doTrade,1,-MAXLIMIT);
		else if(strstr(Text,"Buy Short"))
			call(1,doTrade,2,MAXLIMIT);
		else if(strstr(Text,"Close Long"))
			call(1,doTrade,3,MAXLIMIT);
		else if(strstr(Text,"Close Short"))
			call(1,doTrade,4,-MAXLIMIT);
		else if(strstr(Text,"Update Stop")) {
			exitShort(0,priceC(0)+Stop,0);	
			exitLong(0,priceC(0)-Stop,0);	
		}
		OrderLimit = 0;
	}
}
...
#endif
#ifdef PERCENTSTOP	
	Stop = Trail = 0.01*priceC(0)*slider(2,0,0,40,"Stop %",0);
#else
...
_POS(20);
	...
	if(!is(LOOKBACK) && is(SPONSORED))
		printf("\nVol %s  Spr %s  MCost %s",sftoa(marketVol(0),2),sftoa(Spread,2),sftoa(MarginCost,2));
#endif
#ifdef LOG_ALL
	if(once(!is(LOOKBACK))) {
